...
REGULAREXPRESSION(PhoneNumerRegularExpression, @"\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[3458]+\\d{9}|\\d{8}|\\d{7}")
...
REGULAREXPRESSION(ArrowRegularExpression, @"-->([\\u4e00-\\u9fa5\\w\\-]+)")
...
static NSArray * kAllRegexps() {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _allRegexps = @[kURLRegularExpression(),kPhoneNumerRegularExpression(),kEmailRegularExpression(),kUserHandleRegularExpression(),kHashtagRegularExpression(),kArrowRegularExpression()];
    });
    return _allRegexps;
}
...
{
    MLDataDetectorTypes const allDataDetectorTypes[] = {MLDataDetectorTypeURL,MLDataDetectorTypePhoneNumber,MLDataDetectorTypeEmail,MLDataDetectorTypeUserHandle,MLDataDetectorTypeHashtag,MLDataDetectorTypesArrow};
