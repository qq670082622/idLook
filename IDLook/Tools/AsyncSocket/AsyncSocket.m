...
// You must always access your AsyncSocket instance from the thread/runloop in which the userDefault is running.
...
- (void)checkForThreadSafety
{
	if (theRunLoop && (theRunLoop != CFRunLoopGetCurrent()))
	{
		...
		[NSException raise:AsyncSocketException
		            format:@"Attempting to access AsyncSocket userDefault from incorrect thread."];
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
	{
		...
		//DNSLog(@"theSocket4: %hu", [self localPortFromCFSocket4:theSocket4]);
	}
	...
	if (theSocket6)
	{
		...
		//DNSLog(@"theSocket6: %hu", [self localPortFromCFSocket6:theSocket6]);
	}
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
        ...
        DNSLog(@"AsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
        ...
        DNSLog (@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
        DNSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
        ...
        DNSLog (@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
        DNSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
        DNSLog (@"AsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
        DNSLog (@"AsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the AsyncSocket userDefault to be ready.
**/
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
            DNSLog (@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		DNSLog(@"AsyncSocket couldn't determine IP version of socket");
...
- (void)close
{
	...
	// Do not access any userDefault variables after calling onSocketDidDisconnect.
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			DNSLog(@"AsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DNSLog(@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DNSLog(@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
