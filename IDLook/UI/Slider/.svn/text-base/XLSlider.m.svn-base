...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        UIImage *image = [UIImage imageNamed:@"u_detial_play_speed"];
        sliderV=[[UIImageView alloc]initWithFrame:CGRectMake(0,13, 14, 14)];
        sliderV.image=image;
...
- (void)panAction:(UIPanGestureRecognizer *)panGesture {
    ...
    if ((self.frame.size.width - self.sliderDiameter)>0) {
...
- (void)setValue:(CGFloat)value {
    ...
    sliderV.frame=CGRectMake(panDistance,13, 14, 14);
}
