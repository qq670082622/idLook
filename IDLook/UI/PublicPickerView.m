...
@interface PublicPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation PublicPickerView
...
- (void)showWithPickerType:(PickerType)type withDesc:(NSString *)string
{
    self.type=type;
    self.dataS = [PublicManager getPickerDataWithType:type];
    self.selectStr =[NSString stringWithFormat:@"%@",[self.dataS firstObject]];
    
    NSString *value = [NSString stringWithFormat:@"%@",string];
    
    if ([string integerValue]==0) {
        if (type!=PickerTypeNationality && type!=PickerTypeLanguage && type!=PickerTypeOccupation && type!=PickerTypeSex &&type!=PickerTypePostion &&type!=PickerTypeMastery) {
            NSInteger index = (NSInteger)self.dataS.count/2;
            [self.pickerView selectRow:index inComponent:0 animated:NO];
            self.selectStr = self.dataS[index];
        }
    };
...
{
    self.didSelectBlock(self.selectStr);
    [self hide];
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    NSString *str;
    if (self.type==PickerTypeShoeSize) {
        str=@"码";
        if ([self.dataS[row] integerValue]==0) {
            str=@"";
        }
    }
    else if (self.type==PickerTypeWeight)
    {
        str=@"kg";
        if ([self.dataS[row] integerValue]==0) {
            str=@"";
    ...
    else if(self.type==PickerTypeHeight || self.type==PickerTypeWaist || self.type==PickerTypeBust || self.type==PickerTypeHipline)
    {
        str=@"cm";
        if ([self.dataS[row] integerValue]==0) {
            str=@"";
    ...
    genderLabel.text = [NSString stringWithFormat:@"%@%@",self.dataS[row],str];
