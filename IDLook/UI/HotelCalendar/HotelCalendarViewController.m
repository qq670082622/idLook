...
#define kBlueColor [UIColor colorWithRed:255/ 255.0 green:74 / 255.0 blue:87 / 255.0 alpha:1]

static const CGFloat kHeaderViewH = 70.f;
static const CGFloat kFooterViewH = 80.f;
static const CGFloat kCalendarViewH = 500.f; // 70 + 50 + 50 * 6 + 50
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (shiftIndex >= containPreDays) {
        ...
        //******自己加的*****//
        NSDate *nextDate = [NSDate dateWithTimeInterval:24 * 60 * 60*9 sinceDate:yyMMDDDate];
        if (_toDay>0) {
            nextDate = [NSDate dateWithTimeInterval:24 * 60 * 60*_toDay sinceDate:yyMMDDDate];
        }
        self.selectToDate = nextDate;
        //***********//
...
- (void)setupFooterViews {
    _footerFinishView = [[UIView alloc] initWithFrame:CGRectMake(0, kCalendarViewH - kFooterViewH, CGRectGetWidth(self.view.frame), kFooterViewH)];
    ...
    UILabel *tip = [[UILabel alloc] initWithFrame:CGRectMake(0, 0.5, CGRectGetWidth(self.view.frame), 20)];
    tip.text = @"该保险项目默认为10天保期";
    tip.font = [UIFont systemFontOfSize:14 weight:UIFontWeightMedium];
    tip.textColor = [UIColor  colorWithRed:255.f / 255.f green:74.f / 253.f blue:87.f / 255.f alpha:1];
    tip.textAlignment = NSTextAlignmentCenter;
    [_footerFinishView addSubview:tip];
    
    UIButton *finishBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    finishBtn.frame = CGRectMake(15, 30, CGRectGetWidth(self.view.frame) - 30, 40);
