...
@implementation WorkTypeSelectV
...
- (void)showWithSelectArray:(NSArray *)array withType:(NSInteger)type withTitle:(NSString *)title
{
    ...
    for (int i =0; i<array.count; i++) {
        if ([self.dataS containsObject:array[i]]) {
            [self.selectArray addObject:array[i]];
        }
    }
...
-(void)clickType:(UIButton*)sender
{
    ...
    else
    {
        ...
        if (self.selectArray.count>=2&&sender.selected==NO) {
            [SVProgressHUD showImage:nil status:@"最多只能选择两个关键词"];
            return;
        }
...
-(void)getDataWithType:(NSInteger)type
{
    ...
    for (int i=0; i<array1.count; i++) {
        ...
        if (cateid==[UserInfoManager getUserAgeType]) {
            if (type==0) {  //作品类型
                array =dicA[@"attribute"][@"workType"];
            }
            else if (type==1)  //作品关键词
            {
                array =dicA[@"attribute"][@"workKeyType"];
            }
            else if (type==2)   //角色
            {
                array =dic[@"filmActorRole"];
            }
        }
    }
