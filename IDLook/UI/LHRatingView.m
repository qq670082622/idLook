...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.widDistance = 17.0f;//星星间距
        self.heiDistance = 0.0f;//离顶部间距
        ...
        eachWidth = 21;//(CGRectGetWidth(self.frame)-(starNumber-1)*self.widDistance)/starNumber; //星星宽
        CGFloat height = 21;//CGRectGetHeight(self.frame)-2*self.heiDistance;//星星高
        for (int i = 0; i < starNumber; i++) {
            UIImage * grayImg = [UIImage imageNamed:@"icon_star_unselect"];
            ...
            UIImage * foreImg = [UIImage imageNamed:@"icon_star_select"];
...
- (void)changeStarForeViewWithPoint:(CGPoint)point
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(ratingView:score:andType:)])
    {
        [self.delegate ratingView:self score:self.score andType:_type];
    }
}
