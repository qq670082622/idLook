...
@interface MakeupWayPopV()
@property (nonatomic,strong)UIView *maskV;
@property (nonnull,strong)NSArray *dataS;
@end

@implementation MakeupWayPopV


- (id)init
{
    self = [super initWithFrame:CGRectMake(0, UI_SCREEN_HEIGHT, UI_SCREEN_WIDTH, 320)];
    if(self){
...
{
    self.dataS = [PlaceOrderModel getShotWay];
    ...
    self.frame = CGRectMake(0, UI_SCREEN_HEIGHT-320, UI_SCREEN_WIDTH, 320);
...
- (void)hide
{
    ...
    self.frame = CGRectMake(0, UI_SCREEN_HEIGHT, UI_SCREEN_WIDTH, 320);
    [UIView commitAnimations];
}
...
- (void)confirmSelected
{
    ...
    if (model.isChoose==NO) {
        [SVProgressHUD showImage:nil status:@"请选择定妆场地"];
        return;
    }
...
- (void)initUI
{
    ...
    for (int i=0; i<self.dataS.count; i++) {
        ...
        MakeupPopCell *cell = [[MakeupPopCell alloc]init];
...
-(void)cellTapAction:(UITapGestureRecognizer*)tap
{
    for (int i=0; i<self.dataS.count; i++) {
        ...
        MakeupPopCell *cell = [self viewWithTag:1000+i];
        [cell reloadUIWithOrderM:model];
    }
}
...
-(void)reloadUIWithOrderM:(OrderStructM *)model
{
    ...
    NSString *title = @"";
    NSString *desc =@"";
    if ([model.title isEqualToString:@"平面影棚"]) {
        desc=@"/3小时";
    ...
    if (model.isChoose==NO) {
        [attStr1 addAttributes:@{NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#999999"]} range:NSMakeRange(model.title.length,title.length)];
    }
    [attStr1 addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0]} range:NSMakeRange(model.title.length,title.length)];
    ...
    [attStr2 addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0]} range:NSMakeRange(model.price.length+1,desc.length)];
