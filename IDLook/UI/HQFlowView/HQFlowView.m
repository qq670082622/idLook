...
@property(nonatomic,assign) NSInteger QN_sec;
@end
...
- (void)autoNextPage {
    ...
    if (_QN_sec==1) {
        _QN_sec = 0;
        NSInteger uid = [[UserInfoManager getUserUID] integerValue];
        if (uid>0) {
            NSDictionary *qnDic = @{
                                    @"userId":@(uid)
                                    };
            [AFWebAPI_JAVA askRoomInviteWithArg:qnDic callBack:^(BOOL success, id object) {
                if (success) {
                    NSDictionary *body = object[@"body"];
                    NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
                    [center postNotificationName:@"presentToQN" object:body userInfo:nil]; //obj应该放body
                }
            }]; 
        }
